  ...
  return (
    <>
      ...
      <div className="container mt-5 order-details-container">
        ...
        <p>
          <strong>{t('orderDetails.reference')}</strong> {order.reference}
        </p>
        <p>
          <strong>{t('orderDetails.status')}</strong>{' '}
          {statusTranslations[order.status] || order.status}
        </p>
        <p>
          <strong>{t('orderDetails.date')}</strong>{' '}
          {formatDate(new Date(order.date)) || 'Non spécifiée'}
        </p>
        <p>
          <strong>{t('orderDetails.paymentMethod')}</strong>{' '}
          {order.paymentMethod?.last4
            ? `**** **** **** ${order.paymentMethod.last4}`
            : t('orderDetails.noPaymentMethod')}
        </p>
        <p>
          <strong>{t('orderDetails.address')}</strong> {order.address}
        </p>
          ...
          <tbody>
            {order.orderLines.map((line, index) => (
              <tr key={index}>
                ...
                <td>
                  {line.quantity}
                  {order.reference.startsWith('sub') && ' ' + t('orderDetails.month')}
                </td>
                <td>{formatPrice(line.price)}</td>
                <td>
                  {line.promotionPrice
                    ? formatPrice(line.promotionPrice)
                    : t('orderDetails.lineNoPromotion')}
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="mt-4">
          <p>
            <strong>{t('orderDetails.total')}</strong> {formatPrice(order.total)}
          </p>
          <p>
            <strong>{t('orderDetails.promotion')}</strong>{' '}
            {order.promotion ? formatPrice(order.promotion) : t('orderDetails.noPromotion')}
          </p>
        </div>
        ...
        {order.reference.startsWith('sub') && order.status === 'active' && (
          <>
            <button onClick={() => setShowCancelModal(true)} className="btn btn-danger mt-3 mb-2">
            ...
            <Modal show={showCancelModal} onHide={() => setShowCancelModal(false)}>
              ...
              <Modal.Body>{t('orderDetails.modal.body')}</Modal.Body>
              <Modal.Footer>
                ...
                <Button variant="danger" onClick={handleCancelSubscription} disabled={isCancelling}>
                  {isCancelling
                    ? t('orderDetails.modal.cancelling')
                    : t('orderDetails.modal.confirm')}
                </Button>
              </Modal.Footer>
            </Modal>
          </>
